...
def train_model(X_train, y_train, X_val, y_val, X_test, y_test, num_classes=2, 
    ...
    def _train_model(X_train, y_train, X_val, y_val, X_test, y_test, seed=seed):
        ...
        model = build_classifier(input_dim, num_classes).to(device)
        ...
        for epoch in range(num_epochs):
            ...
            for x_batch, y_batch in train_loader:
                ...
                optimizer.step()
                scheduler.step()
                total_loss += loss.item() * y_batch.size(0) #mean loss multiplied by batch size to give the loss sum
            ...
            with torch.no_grad():
                ...
                for x_batch, y_batch in val_loader:
                    ...
                    logits = model(x_batch)
                    loss = criterion(logits, y_batch)
                    val_total_loss += loss.item() * y_batch.size(0)
                    preds = logits.argmax(1) #get the highest logit across the classes for each sample
                    correct += (preds == y_batch).sum().item() #.item() when converting tensors to numbers/accumulating metrics
        ...
        model.eval()
        with torch.no_grad():
            logits = model(emb_test.to(device))
